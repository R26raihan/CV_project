...
export default function CreativeBlock({ cv, ui, activeFont }: TemplateProps) {
  return (
    <div class={`${ui.layoutColumns === 1 ? 'flex flex-col' : 'flex'} h-full bg-white transition-all duration-700 min-w-0`} style={{ fontFamily: activeFont }}>
      {/* Sidebar / Top Head */}
      <aside class={`${ui.layoutColumns === 1 ? 'w-full p-16' : 'w-[32%] p-12'} text-white flex flex-col gap-12 relative overflow-hidden transition-all duration-500 shrink-0`} style={{ backgroundColor: ui.accentColor }}>
        ...
        <div class="relative z-10 flex flex-col gap-8">
           <div class="w-32 h-32 rounded-3xl overflow-hidden border-4 border-white/20 shadow-2xl shrink-0 group-hover:scale-105 transition-transform">
             {cv.profileImage ? (
          ...
          <section class="min-w-0">
             ...
             <div class="flex flex-wrap gap-2">
                {cv.skills.map((skill: string) => (
                  <span key={skill} class="px-3 py-1.5 bg-white/10 text-[10px] font-black rounded-lg uppercase tracking-wider backdrop-blur-sm border border-white/10">
                    {skill}
                  </span>
                ))}
             </div>
